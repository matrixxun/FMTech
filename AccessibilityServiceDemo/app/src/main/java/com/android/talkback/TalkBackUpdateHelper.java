...
public class TalkBackUpdateHelper {
    ...
    public TalkBackUpdateHelper(MyAccessibilityService service) {
        mService = service;
    ...
    private boolean needsExploreByTouchHelper(int previousVersion) {
        return (Build.VERSION.SDK_INT == 16) && (previousVersion == 68)
                && !mSharedPreferences.getBoolean(MyAccessibilityService.PREF_FIRST_TIME_USER, true);
    }
    ...
    private static class ExploreByTouchUpdateHelper implements AccessibilityEventListener {
        ...
        public ExploreByTouchUpdateHelper(MyAccessibilityService service) {
            mService = service;
        }
