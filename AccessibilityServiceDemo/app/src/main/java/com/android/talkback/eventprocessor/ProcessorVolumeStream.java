...
public class ProcessorVolumeStream implements AccessibilityEventListener,
        MyAccessibilityService.KeyEventListener, VolumeButtonPatternDetector.OnPatternMatchListener {
    ...
    private void handleBothVolumeKeysLongPressed() {
        if (MyAccessibilityService.isServiceActive() && switchTalkBackActiveStateEnabled()) {
    ...
    private void handleSingleTap(int button) {
        if (MyAccessibilityService.isServiceActive() && attemptNavigation(button)) {
            return;
        }
