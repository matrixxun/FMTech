...
public class AccessibilityTutorialActivity extends Activity {
    ...
    protected void onPause() {
        ...
        final MyAccessibilityService service = MyAccessibilityService.getInstance();
        if (service != null) {
    ...
    protected void onResume() {
         ...
         * enabled).
         */
        final int serviceState = MyAccessibilityService.getServiceState();
        /*
    ...
    SpeechController getSpeechController() {
        final MyAccessibilityService service = MyAccessibilityService.getInstance();
        if (service == null) {
            LogUtils.log(Log.ERROR, "Failed to get MyAccessibilityService instance.");
            return null;
        }
    ...
    private final MyAccessibilityService.ServiceStateListener mServiceStateListener =
            new MyAccessibilityService.ServiceStateListener() {
        @Override
        public void onServiceStateChanged(int newState) {
            if (newState == MyAccessibilityService.SERVICE_STATE_INACTIVE) {
                // If the service dies while the tutorial is active, exit.
                finish();
            } else if (newState == MyAccessibilityService.SERVICE_STATE_SUSPENDED) {
                stopRepeating();
