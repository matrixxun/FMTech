...
public class SpeechController {
    ...
    public SpeechController(MyAccessibilityService context,
                            FeedbackController feedbackController) {
        if (feedbackController == null) throw new IllegalStateException();

        mService = context;
        mService.addServiceStateListener(new MyAccessibilityService.ServiceStateListener() {
            @Override
            public void onServiceStateChanged(int newState) {
                if (newState == MyAccessibilityService.SERVICE_STATE_ACTIVE) {
